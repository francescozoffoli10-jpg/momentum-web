...
// ── Open status pill ──────────────────────────────────────────────────────────
// Helper: find today's hours string from tenant.hours array
function getTodayHours(tenant: Tenant): string | null {
  ...
  const nm: Record<string, number> = {
    ...
    jue: 4, jueves: 4,
    vie: 5, viernes: 5,
    sab: 6, sabado: 6, sábado: 6, sabados: 6, sábados: 6,
  }
  for (const row of tenant.hours) {
    const t = row.days.toLowerCase().replace(/[^a-záéíóúüñ\s]/gi, ' ')
    // Range pattern: "lunes a viernes"
    const m = t.match(/([a-záéíóúüñ]+)\s+a\s+([a-záéíóúüñ]+)/)
    if (m) {
      const s = nm[m[1]], e = nm[m[2]]
      if (s !== undefined && e !== undefined) {
        const inRange = s <= e ? idx >= s && idx <= e : idx >= s || idx <= e
        if (inRange) return row.hours
      }
    }
    // Individual day name present in row
    for (const [k, v] of Object.entries(nm)) {
      if (v === idx && t.split(/\s+/).some((w: string) => w === k)) return row.hours
    }
  }
  return null
}
...
function OpenBadge({ tenant }: { tenant: Tenant }) {
  ...
  const todayHours = getTodayHours(tenant)
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 16 }}>
      <div style={{
        ...
        <div style={{ width: 5, height: 5, borderRadius: '50%', background: status ? '#22c55e' : '#ef4444' }} />
        <span style={{ fontSize: 9, fontWeight: 600, letterSpacing: '0.1em', color: status ? '#16a34a' : '#dc2626', textTransform: 'uppercase' }}>
          {status ? 'Abierto ahora' : 'Cerrado ahora'}
        </span>
      </div>
      {todayHours && (
        <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.07em', fontWeight: 300 }}>
          · HORARIO HOY: {todayHours}
        </span>
      )}
    </div>
  )
          ...
          <div className={`tenant-hero-grid${tenant.videoUrl ? ' has-video' : ''}`}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                ...
                {tenant.menuUrl && (
                  <a href={tenant.menuUrl} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '11px 20px', background: 'var(--a)', color: '#fff', fontSize: 12, fontWeight: 600, letterSpacing: '0.06em', borderRadius: 4, transition: 'opacity 0.18s, transform 0.18s', textDecoration: 'none' }}
                ...
                {tenant.phone && (
                  <a href={`tel:${tenant.phone.replace(/\D/g, '')}`}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '11px 20px', background: 'rgba(255,255,255,0.06)', border: '0.5px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.8)', fontSize: 12, fontWeight: 500, letterSpacing: '0.04em', borderRadius: 4, transition: 'background 0.18s, transform 0.18s', textDecoration: 'none' }}
                    onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.background = 'rgba(255,255,255,0.12)'; el.style.transform = 'translateY(-1px)' }}
                    onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.background = 'rgba(255,255,255,0.06)'; el.style.transform = 'translateY(0)' }}
                  >
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                ...
                {tenant.whatsapp && (
                  <a href={`https://wa.me/${tenant.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '11px 20px', background: '#25D366', color: '#fff', fontSize: 12, fontWeight: 600, letterSpacing: '0.06em', borderRadius: 4, transition: 'opacity 0.18s, transform 0.18s', textDecoration: 'none' }}
                    onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.opacity = '0.88'; el.style.transform = 'translateY(-1px)' }}
                    onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.opacity = '1'; el.style.transform = 'translateY(0)' }}
                  >
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, marginTop: 20, paddingTop: 16, borderTop: '0.5px solid rgba(255,255,255,0.07)', fontSize: 11 }}>
                {tenant.phone && (
                  <span>
                    ...
                    <span style={{ color: 'rgba(255,255,255,0.35)' }}>{tenant.phone}</span>
                  </span>
                )}
                <span>
                  <span style={{ color: 'var(--a)', fontWeight: 600, letterSpacing: '0.1em', marginRight: 6 }}>UBICACIÓN</span>
                  <span style={{ color: 'rgba(255,255,255,0.35)' }}>{site.name}, {site.city}</span>
                </span>
                {tenant.local && (
                  <span>
                    <span style={{ color: 'var(--a)', fontWeight: 600, letterSpacing: '0.1em', marginRight: 6 }}>LOCAL</span>
                    <span style={{ color: 'rgba(255,255,255,0.35)' }}>{tenant.local}</span>
                  </span>
                )}
              </div>
            </div>
