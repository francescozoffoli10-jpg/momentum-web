...
| 2026-06-02 | Teatro Cartelera — Sanity CMS schemas | Created `src/sanity/schemas/teatroShow.ts` (individual show/función schema: title, subtitle, genre dropdown, duration, dates array, image, ticketUrl, isActive, featured, order). Created `src/sanity/schemas/teatroConfig.ts` (singleton schema for all static teatro page content: hero, identity, stats, specs, bistro, contact, videoFile). Both registered in `schemas/index.ts`. Studio structure updated in `config.ts` with Funciones list + teatroConfig singleton item. |
| 2026-06-02 | Teatro Cartelera — page integration | Added `TeatroShow`, `TeatroShowDate`, `TeatroStat`, `TeatroSpec`, `TeatroConfig` interfaces to `types.ts`. Added `TEATRO_SHOWS_ACTIVE` + `TEATRO_CONFIG` GROQ queries to `queries.ts`. Added `fetchTeatroShows()` + `fetchTeatroConfig()` to `fetch.ts`. Updated `teatro/page.tsx`: parallel fetch, CMS-first with hardcoded fallbacks for all content, `ShowCard` component with genre pill/featured badge/upcoming dates, empty state. |
| 2026-06-02 | Teatro category pills — 5 directory pages | Added Teatro pill to `sectionLinks` in all 5 Pinares directory pages: `gastronomia/page.tsx`, `comercios/page.tsx`, `servicios/page.tsx`, `torre-medica/page.tsx`, `ofiplaza/page.tsx`. |
| 2026-06-02 | Teatro video showreel — CMS + website | Added `videoFile` field (type: file, accept: video/*) to `teatroConfig` Sanity schema. Added `"videoUrl": videoFile.asset->url` to `TEATRO_CONFIG` GROQ query. Added `videoUrl?: string` to `TeatroConfig` interface. Added full-width cinematic video section to `teatro/page.tsx` (autoplay muted loop, between Identity and Stats, only renders when `videoUrl` present, dark vignette top+bottom, "Showreel" label overlay). Commits `a79acc06`, `b3a3641a`, `027eb660`, `d2df5247`. |
| 2026-06-02 | Teatro Identity section — video replaces static image | Moved video showreel into the Identity section right column (`alignItems: 'stretch'` grid). Video fills exactly the height of the text column via `position: absolute; inset: 0`. Falls back to `identityImage` when no `videoUrl`. Removed standalone video section. Commit `5d05b96c`. |
| 2026-06-02 | Cartelera Teatro — 5 shows seeded | Converted 5 JPG poster images (6000×3375) to WebP (1920×1080, 76–236 KB). Uploaded to Sanity Assets API. Created `teatroShow` Sanity docs: Adictas a vos (comedia, Jun–Jul), El Principito (familiar, Jun–Sep), Rebelión en la granja (drama, Aug–Sep), Caminito del Mar (familiar, Nov), Un Cuento de Navidad (familiar, Dec). All with full date arrays. Transaction `2L5iqaQa3UmcCheddoE13I`. |
| 2026-06-02 | Pinares homepage — Teatro hero from CMS | Made `PinaresHomePage` async, added `fetchTeatroConfig()`, uses `cfg?.heroImage` as teatro section background. Appends `?w=1920&q=80&auto=format` to Sanity CDN URLs for optimized loading. Commit `27d50b231da1`. |
| 2026-06-02 | Footer cross-site accent colors fixed | `Footer.tsx` `OTHER_SITES` had wrong accents: Escazú `#7A2B8A` (purple) → `#56717A` (slate blue-grey); Pinares `#1A4A7A` (navy) → `#4F5B3E` (olive green). Commit `4cffa609592f`. |
| 2026-06-02 | TenantDetailPage — today's hours + Llamar + Mapa + info strip | Added `getTodayHours()` helper that parses day ranges. `OpenBadge` now shows `· HORARIO HOY: HH:MM am – HH:MM pm` inline. CTA order: Ver Menú (accent primary) → Compartir → Llamar al XXXX → Ver en Mapa → WhatsApp. Added quick info strip below CTAs: `CONTACTO · UBICACIÓN · LOCAL`. Affects all 3 sites. Commit `c939053f339c`. |
| 2026-06-02 | FeaturedTenants — fix photo + logo double-path bug | Photo `src` was prepending `/sites/${siteId}/photos/` to paths that already started with `/sites/…`. Added `startsWith('/')` guard. Same fix applied to both logo `src` lines. Commits `c80d98f7a1ea`, `171b40bdbb1f`. |
...
| ~~TEATRO VIDEO~~ | ~~Add video showreel to teatro page~~ | ✅ Done 2026-06-02 — `videoFile` field in Sanity schema + query + type; cinematic section in teatro page |
