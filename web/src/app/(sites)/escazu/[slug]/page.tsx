...
export async function generateStaticParams() {
  const sanitySlugs = await fetchTenantSlugs('escazu')
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const sanityTenant = await fetchTenantBySlug('escazu', slug)
  const tenant: Tenant | undefined = sanityTenant ?? getTenant(slug)
  if (!tenant) return {}
  const ogImage = tenant.logo?.startsWith('http')
    ? tenant.logo
    : `${CANONICAL_BASE}/sites/escazu/logos/${tenant.logo}`
  return {
    title: tenant.name,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const sanityAll = await fetchTenantsBySite('escazu')
